...
interface PremiumToggleProps {
  ...
  checked?: boolean          // controlled mode — overrides internal state
  onChange?: (checked: boolean) => void
  label?: string
}

export function PremiumToggle({ defaultChecked = false, checked, onChange, label }: PremiumToggleProps) {
  const isControlled = checked !== undefined
  const [internalChecked, setInternalChecked] = useState(defaultChecked)
  const [isPressed, setIsPressed] = useState(false)

  // In controlled mode use the prop, otherwise use internal state
